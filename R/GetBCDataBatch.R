#' Download BC Data layers in batch.
#'
...
#' the name or ID of a resource or the URL or a BC Geographic Warehouse BCGW
#' name. The 'geom_ext' represents the name of the SF object as a string for
#' which to intersect the layer data with, which must be present in the global
#' environment. The 'filter_exp' field represents the filter expression as a
#' string to be passed to bcdata::filter evaluated by bcdata::CQL.
#' @return A list of the resulting BC Data layers defined by 'layer_tbl'.
#' @export
get_bcdata_batch<-function(layer_tbl)
{
  ...
  for(row in c(1:nrow(layer_tbl)))
  {
    ...
    #No geometry or filter is provided, just DL
    ...
    #If only a geometry is provided, intersect and DL
    ...
    #If both geometry and filter expression are provided, intersect filter and DL
    ...
    #If only filter is provided, filter and DL
